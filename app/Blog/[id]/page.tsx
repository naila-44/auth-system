...
export default function PostDetailPage() {
  ...
  if (loading) return <p className="text-center mt-10 text-[#7f5539]">Loading post...</p>;
  if (!post) return <p className="text-center mt-10 text-[#7f5539] font-medium">Post not found.</p>;
      ...
      <div className="max-w-3xl w-full flex-1 space-y-4 overflow-y-auto">
        <div className="bg-white/70 backdrop-blur-md border border-[#e6ccb2]/50 shadow-xl rounded-3xl overflow-hidden">
          ...
          <div className="p-6 space-y-3">
            ...
            <p className="text-[#5a3825]/95 leading-relaxed whitespace-pre-line">{post.content}</p>
          </div>
        </div>
                ...
                >
                  ...
                  <div className="flex gap-2 mt-1">
                    {["❤️", "👍", "😮"].map((emoji) => (
                      <button
                        key={emoji}
                        onClick={() => socket.emit("reactComment", { commentId: c.id, emoji })}
